...
export function useScreenType<B extends Record<string, number> = typeof defaultBreakpoints, K extends keyof B = keyof B>(
	breakpointsArg?: B,
): {
	...
	const queries = sorted.map(([k, v], i) =>
		i === 0 ? { key: k, query: `(max-width: ${v - 1}px)` } : { key: k, query: `(min-width: ${sorted[i - 1][1]}px) and (max-width: ${v - 1}px)` },
	);
