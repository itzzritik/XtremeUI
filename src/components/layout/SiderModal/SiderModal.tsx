...
import type { TSiderModalProps } from "./types";

export const SiderModal = forwardRef<HTMLDivElement, TSiderModalProps>((props, ref) => {
	const { className, children, title, icon, primaryButtonProps = {} as TButtonProps, secondaryButtonProps = {} as TButtonProps } = props;

	const { setSiderMode } = useXData();

	const SiderModalClsx = clsx("xtrSiderModal", className);
	...
	return (
		<div ref={ref} className={SiderModalClsx} role="dialog">
			...
			<div className="footer">
				<Button type="primary" {...primaryButtonProps} className={clsx("primaryButton", primaryButtonProps?.className)} />
				<Button type="secondary" {...secondaryButtonProps} className={clsx("secondaryButton", secondaryButtonProps?.className)} />
			</div>
		</div>
	);
});
